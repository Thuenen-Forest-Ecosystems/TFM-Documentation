...
function buildColDefs() {
  ...
  //  { field: "responsible_state", headerName: "Land", filter: "agTextColumnFilter" },
    ...
    { field: "wald_vorgaenger_text", headerName: "Waldentscheid_Vorgänger", filter: "agNumberColumnFilter" },
    { field: "begehbar2027", headerName: "Begehbarkeit_2027", filter: "agNumberColumnFilter" },
    { field: "begehbar_vorgaenger", headerName: "Begehbarkeit_Vorgänger", filter: "agNumberColumnFilter" }
  ];
}
...
async function fetchRecordChangesInBatches(batchSize = 1000) {
  ...
  try {
    ...
    while (hasMore) {
      ...
      let query = supabase
        .from('v_stats_forest_status_changed')
        .select('lil, cluster_name, plot_name,  troop_name, kt, completed_at_troop, wald2027_text, wald_vorgaenger_text, begehbar2027, begehbar_vorgaenger, responsible_state, responsible_administration, responsible_provider, responsible_troop')
        .or(orFilters.join(','))
...
function onBtnExport1() {
  gridApi1.value?.exportDataAsCsv({
    fileName: `CI-Statistik_forest_status_changed_${new Date().toISOString().slice(0, 10)}.csv`
  });
}
</script>
